...
# Extract matching familiar / novel test session associated with behavior_session_id
def get_test_sessions(cache, test_session_behavior_id = 1093668878):
    ecephys_session_table = cache.get_ecephys_session_table()
    ecephys_session_table = ecephys_session_table.reset_index()

    # Choose a suitable example session and pull behavior data for the session
    test_session_mouse_id = ecephys_session_table.loc[ecephys_session_table.behavior_session_id==test_session_behavior_id, "mouse_id"].values[0]

    test_session1 = ecephys_session_table.loc[ecephys_session_table.mouse_id==test_session_mouse_id].iloc[0]
    test_session2 = ecephys_session_table.loc[ecephys_session_table.mouse_id==test_session_mouse_id].iloc[1]

    test_session1_experience_level = test_session1["experience_level"]
    test_session1_ecephys_id = test_session1["ecephys_session_id"]
    test_session1_behavior_id = test_session1["behavior_session_id"]

    test_session2_experience_level = test_session2["experience_level"]
    test_session2_ecephys_id = test_session2["ecephys_session_id"]
    test_session2_behavior_id = test_session2["behavior_session_id"]

    print(f'Test session 1:\n ecephys_id: {test_session1_ecephys_id}, behavior_id: {test_session1_behavior_id} ({test_session1_experience_level}) -- mouse: {test_session_mouse_id}.')
    print(f'Test session 2:\n ecephys_id: {test_session2_ecephys_id}, behavior_id: {test_session1_behavior_id} ({test_session2_experience_level}) -- mouse: {test_session_mouse_id}.')

    # Load ephys data
    test_session1 = cache.get_ecephys_session(test_session1_ecephys_id)
    test_session2 = cache.get_ecephys_session(test_session2_ecephys_id)

    test_session_list                   = [test_session1, test_session2]
    test_session_ecephys_id_list        = [test_session1_ecephys_id, test_session2_ecephys_id]
    test_session_experience_level_list  = [test_session1_experience_level, test_session2_experience_level]

    return test_session_list, test_session_ecephys_id_list, test_session_experience_level_list
...
def get_euclidean_distance(population_vectors, ref_vectors=None, mode='compare_first'):
    # modes: 'compare_first', 'compare_previous'

    if ref_vectors is None:
        ref_vectors=population_vectors
    ...
    elif mode=='compare_ref':
        for ii, (pv_ii, pv_jj) in enumerate(zip(population_vectors, ref_vectors)):
            dist_i = np.linalg.norm(pv_ii - pv_jj)
            dist_all.append(dist_i)
    elif mode=='compare_previous':
        for ii, (pv_ii, pv_jj) in enumerate(zip(population_vectors[1:], ref_vectors[:-1])):
            dist_i = np.linalg.norm(pv_ii - pv_jj)
            dist_all.append(dist_i)
...
def mov_mean(x, window_size):
    """Compute moving average with a specified window size."""
